...
```
# 1. Find what's ready (dependencies satisfied, not in progress)
Use the feature_get_ready tool with limit=5

# 2. Pick the top feature and atomically claim it
Use the feature_claim_and_get tool with feature_id={top_feature_id}
```

`feature_claim_and_get` is atomic — it marks the feature in-progress and returns its full details in one operation. If another agent claimed it first, you'll get an error; call `feature_get_ready` again to get the next available one.
...
# 2. Discover what's ready to work on
feature_get_ready with limit=5

# 3. Atomically claim a feature and get its full details
feature_claim_and_get with feature_id={id}
...
- Use `feature_get_ready` to discover work, then `feature_claim_and_get` to claim it
- Do NOT use `feature_get_by_id` for browsing — only use it if you already know the ID
- Do NOT query all features, list by category, or explore the backlog

**Claim one feature, implement it fully, mark it passing. Repeat.**
