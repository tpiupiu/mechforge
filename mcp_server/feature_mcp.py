...
@mcp.tool()
def feature_get_in_progress() -> str:
    """Get all features currently marked as in-progress.

    Use this at session start to detect features stuck in-progress from a
    previous interrupted session. Any feature returned here should be investigated:
    either continue the work or call feature_clear_in_progress to re-queue it.

    Returns:
        JSON with: features (list), count (int)
    """
    session = get_session()
    try:
        ...
        return json.dumps({
            "features": [f.to_dict() for f in stuck],
            "count": len(stuck)
        })
    finally:
        session.close()
